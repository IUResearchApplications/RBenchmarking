...
benchmark_name <- "cholesky factorization"
# The chol routine calls potrf which requires about (N^3) / 3 operations
sizes <- c(40000)
...
for (j in 1:num_sizes) {
   ...
   for (i in 1:(num_runs[j]+num_warmup_runs[j])) {
      ...
      # Generate a more numerically stable (smaller condition number) matrix
      a <- crossprod(new("dgeMatrix", x = Rnorm(s*s), Dim = as.integer(c(s,s))))
