...
a benchmark function field which specify functions for allocating the
data for the microbenchmark and the function that performs the
timing of functionality being tested.  The microbenchmark definition classes also have
parameters used to specify dimensions of the data sets to be used with the
microbenchmarks.  See the object documentation for the microbenchmark
definition classes for specifics on the class fields and how they should
be populated.
...
decomposition.  The table below lists each name of each microbenchmark supported
along with a brief description of the functionality it tests.

| Microbenchmark name          | Functionality tested         | Kernel functions   |
|:-----------------------------|:-----------------------------|:-------------------|
| choleksy                     | Cholesky factorization       | chol               |
| crossprod                    | Matrix cross product         | crossprod          |
| determinant                  | Matrix determinant           | determinant        |
| eigen                        | Eigendecomposition           | eigen              |
| solve                        | Linear solve                 | solve              |
| lsfit                        | Least squares fit            | lsfit              |
| deformtrans                  | Transpose, reshape, and retranspose matrix | t    |
...
that is microbenchmarked.

| Microbenchmark name          | Functionality tested                | Problem Domain Description                                         | Kernel function  |
|:-----------------------------|:------------------------------------|:-------------------------------------------------------------------|:-----------------|
| matvec_laplacian7pt_100      | Sparse matrix-vector multiplication | Laplacian 7-point stencil applied to 100x100x100 grid              | %*%              |
| matvec_laplacian7pt_200      | Sparse matrix-vector multiplication | Laplacian 7-point stencil applied to 200x200x200 grid              | %*%              |
| matvec_ca2010                | Sparse matrix-vector multiplication | Undirected weighted graph from congressional redistricting         | %*%              |
| cholesky_ct20stif            | Sparse Cholesky factorization       | Stiffness matrix from structural                                   | Matrix::Cholesky |
| cholesky_Andrews             | Sparse Cholesky factorization       | Eigenvalue problem from computer graphics/vision                   | Matrix::Cholesky |
| cholesky_G3_circuit          | Sparse Cholesky factorization       | Circuit simulation                                                 | Matrix::Cholesky |
| lu_circuit5M_dc              | Sparse LU factorization             | Large circuit DC analysis                                          | Matrix::lu       |
| lu_stomach                   | Sparse LU factorization             | 3D electro-physical model of duodenum                              | Matrix::lu       |
| lu_torso3                    | Sparse LU factorization             | 3D electro-physcial model of torso                                 | Matrix::lu       |
| qr_Maragal_6                 | Sparse QR factorization             | Rank deficient least squares                                       | Matrix::QR       | 
| qr_landmark                  | Sparse QR factorization             | Least squares                                                      | Matrix::QR       |
...
documentation of the function `GenerateClusterData` for more details on how the
clusters are generated.  Two cluster identification functions `pam` and `clara`,
from the `cluster` package provided with the **R** distribution.  The `pam`
function implements the *partitioning around medoids* algorithm.  The `clara`
function is used for large data sets and implements a more efficient algorithm
than that used by `pam`.  See the object documentation for the
`RunMachineLearningBenchmark`, `pam`, and `clara` functions for more details.

| Microbenchmark name        | Functionality tested       | Number of features        | Number of clusters | Number of feature vectors per cluster |
|:---------------------------|:---------------------------|:-------------------------:|:------------------:|:-------------------------------------:|
| pam_cluster_3_7_2500       | `pam` clustering           | 3                         | 7                  | 2500                                  |
| pam_cluster_3_7_5000       | `pam` clustering           | 3                         | 7                  | 5000                                  |
| pam_cluster_3_7_5715       | `pam` clustering           | 3                         | 7                  | 5715                                  |
| pam_cluster_16_33_1213     | `pam` clustering           | 16                        | 33                 | 1213                                  |
| pam_cluster_64_33_1213     | `pam` clustering           | 64                        | 33                 | 1213                                  |
| pam_cluster_16_7_2858      | `pam` clustering           | 16                        | 7                  | 2858                                  |
| pam_cluster_32_7_2858      | `pam` clustering           | 32                        | 7                  | 2858                                  |
| pam_cluster_64_7_5715      | `pam` clustering           | 64                        | 7                  | 5715                                  |
| clara_cluster_64_33_1213   | `clara` clustering         | 64                        | 33                 | 1213                                  |
| clara_cluster_1000_99_1000 | `clara` clustering         | 1000                      | 99                 | 1000                                  |
...
Examples are given in this section to show how to run each benchmark.

## Dense Matrix Microbenchmark Examples

This example runs all of the default dense matrix microbenchmarks, saves the
summary statistics for each microbenchmark in the directory `DenseMatrixResults`,
and saves the data frame returned from the dense matrix benchmark to a
file named `allResultsFrame.RData`.
```
...
# Set a new run identifier
runIdentifier <- "no_transpose"
someResults <- RunDenseMatrixBenchmark(runIdentifier, resultsDirectory, myDenseMatrixMicrobenchmarks)
save(someResults, file="./DenseMatrixResults/someResultsFrame.RData")
```
...
summary statistics for each microbenchmark in the directory `SparseMatrixResults`,
and saves the data frame returned from the dense matrix benchmark to a
file named `allResultsFrame.RData`.
```
# Create the directory where the CSV files and data frames will be saved.
...
summary statistics for each microbenchmark in the directory `MachineLearningResults`,
and saves the data frame returned from the dense matrix benchmark to a
file named `allResultsFrame.RData`.
```
# Create the directory where the CSV files and data frames will be saved.
...
# `cluster` package that generates a data set with 250 features, 50 clusters, and
...
be set.  As another example, the `MKL_NUM_THREADS` environment variable should
be set when the dense matrix benchmarks are tested using an instance of **R**
that is linked to the parallel Intel Math Kernel Library.
