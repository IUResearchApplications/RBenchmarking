...
benchmark_name <- "least squares fit"
#sizes <- c(40000)
sizes <- c(40000)
#sizes <- c(1000, 2000, 4000, 8000, 10000)

if (num_threads == 1) {
   #num_runs <- c(2)       # Number of times the tests are executed for each size
...
for (j in 1:num_sizes) {
   ...
   for (i in 1:(num_runs[j]+num_warmup_runs[j])) {
      ...
      a <- matrix(Rnorm(s*s), nrow=2*s, ncol=s/2)
      b <- matrix(Rnorm(2*s), nrow=2*s, ncol=1)
      
      timing <- system.time({
         c <- lsfit(a, b, intercept=FALSE)
      })[3]
